...
namespace perf
{
    public sealed class GAServiceManager : IServiceManager
    {
        ...
        private void OnPayloadFailed(Payload payload, Exception exception)
        {
            ...
            if (PayloadFailed != null) PayloadFailed.Invoke(this, new PayloadFailedEventArgs(payload, exception.Message));
        }

        private async Task OnPayloadSentAsync(Payload payload, HttpResponseMessage response)
        {
            if (PayloadSent != null) PayloadSent.Invoke(this, new PayloadSentEventArgs(payload, await response.Content.ReadAsStringAsync()));
        }
